...
CREATE TABLE IF NOT EXISTS genome (
  ...
  haplotype_id INT NOT NULL DEFAULT 0,
  hap_origin TEXT NOT NULL DEFAULT 'provided' CHECK(hap_origin IN ('provided', 'parsed', 'assumed')),
  name TEXT NOT NULL,
  description TEXT,
  taxon_id BIGINT,
  UNIQUE(sample_id, haplotype_id),
...
CREATE TABLE IF NOT EXISTS pangenome_genome (
  ...
  PRIMARY KEY (pangenome_id, genome_id)
);
...
CREATE TABLE IF NOT EXISTS path_segment_coordinate (
  ...
  segment_order SMALLINT CHECK(segment_order >= 0),
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE c.relname = 'path_seg_coord_path_segment_idx' AND n.nspname = 'public'
    ) THEN
        CREATE INDEX path_seg_coord_path_segment_idx ON path_segment_coordinate(path_id, segment_id);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
